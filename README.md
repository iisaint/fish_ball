...
### 廠商端操作
1. 在「產品統計與價格」區域，點擊「調整價格」
2. 輸入新的價格（可以調高或調低）
3. 確認後自動同步到所有介面

### 團主端顯示
- **全團統計區**：顯示原始總價、折扣金額、應付金額
- **訂單明細區**：每個團員卡片顯示個人折扣
- **即時更新**：廠商調整價格後，所有金額自動重新計算
- **折疊狀態**：即使折疊也能看到價格調整資訊
...
### 功能說明
廠商可以在「草稿預覽」標籤中，提前查看團主正在編輯但尚未送單的訂單。

### 使用場景
1. **提前了解訂單趨勢**：查看即將到來的訂單內容
2. **庫存規劃**：根據草稿內容提前準備貨源
3. **溝通協調**：發現異常訂單可提前與團主溝通

### 重要提示
- ⚠️ **草稿內容隨時可能變動**，團主可能修改或刪除
- 🔒 **唯讀模式**：無法進行價格調整、確認收單等操作
- 📊 **僅供參考**：請勿依此備貨，正式訂單以送單後為準
- 🔄 **自動轉移**：團主送單後，訂單會自動移至「待處理」標籤
...
- ⚠️ **送單後完全鎖定**：團主一旦送單給廠商，**團主和團員的訂單都會立即鎖定**，防止在確認過程中被修改
- 🔄 **可取消送單**：團主可以取消送單，改回草稿狀態，大家都可以繼續修改
- 🔒 **確認後鎖定**：廠商確認收單後，訂單正式成立
- 🔓 **可取消確認**：廠商可以取消確認，訂單狀態改回「已送單」，團主需取消送單才能修改
- 💡 **流程嚴謹**：送單 → 鎖定 → 需修改時取消送單 → 修改 → 重新送單

## 🔐 安全性說明

### 團主頁面自動驗證

系統採用**自動化 Token 驗證機制**保護團主頁面，防止未授權訪問：

#### 工作原理

1. **創建團購時**
   - 系統自動產生 32 位隨機 Token
   - Token 存儲在 Firebase 和瀏覽器 localStorage
   - 完全自動化，用戶無需任何操作

2. **訪問團主頁面時**
   - 系統自動檢查 localStorage 中的 Token
   - 自動向 Firebase 驗證 Token 是否有效
   - 驗證通過才允許訪問
...
| 情境 | 結果 |
|------|------|
| 從首頁建立團購 | ✅ 自動產生 Token，直接進入 |
| 從歷史記錄進入 | ✅ 自動驗證，直接進入 |
| 手動輸入他人團購 URL | ❌ 無 Token，顯示「無權訪問」|
| 清除瀏覽器數據後進入 | ❌ Token 遺失，需從歷史重新進入 |

#### 注意事項

- 📱 **裝置綁定**：Token 存在 localStorage，只能在創建團購的裝置上訪問
- 🔄 **換裝置**：如需在新裝置上訪問，可改為團員身份加入
- 🧹 **瀏覽器數據**：清除瀏覽器數據會導致 Token 遺失

詳細說明請參考：[LEADER_AUTH_FEATURE.md](./LEADER_AUTH_FEATURE.md)
  ...
  "groups": {
    "groupId123": {
      "info": {
        ...
        "location": "台北市信義區",
        "date": "2026-01-15",
        "leaderToken": "XyZ123abc...", // 32 位團主驗證 Token
        "leaderNotes": "請提早出貨，謝謝", // 團主給廠商的備註
